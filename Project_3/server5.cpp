...
void *server_thread(void *socket_desc){
    ...
    while(byte_count = recv(newsock, buf1,sizeof(buf1),0) > 0){
        ...
        if(pid < 0){
            ...
            if (buf1[0] == 'F'){
                int n = m.direct_List.at(pos).size();
                string msg = "\nSize: ";
                msg += to_string(n);
                ...
                string msg = "";
                myFile f(text, "");
                if(m.direct_List.at(pos).exists(f)) {
                    msg += "1 a file of this name already exists";
                }
                else if(!m.direct_List.at(pos).exists(f)) {
                    msg += "0 file successfully created";
                    m.direct_List.at(pos).fileList.push_back(f);
                }
                else {
                    msg+= "2 other failure occured";
                }
                cout << m.direct_List.at(pos).getDirectoryName() << endl;
                char message[msg.size() + 1];
                msg.copy(message, msg.size() + 1);
                message[msg.size()] = '\0';
                send(newsock, message, strlen(message), 0);
                ...
                string msg = "";
                if(m.direct_List.at(pos).exists(f)) {
                    m.direct_List.at(pos).deleteFile(f);
                ...
                string msg = "";
                if(m.direct_List.at(pos).fileList.size() == 0) {
                    msg += "\nERROR: NO FILES IN DIRECTORY";
                }
                else {
                    msg += m.direct_List.at(pos).fileListing();
                }
                char message[msg.size() + 1];
                msg.copy(message, msg.size() + 1);
                message[msg.size()] = '\0';
                ...
                if(m.direct_List.at(pos).fileList.size() == 0) {
                    ...
                    msg += m.direct_List.at(pos).directoryListings();
                }
                char message[msg.size() + 1];
                msg.copy(message, msg.size() + 1);
                message[msg.size()] = '\0';
                ...
                if(m.direct_List.at(pos).exists(f)) {
                    msg += "\n0 ";
                    msg += to_string(m.direct_List.at(pos).fileSize(f));
                    msg += " File Data:";
                    msg += m.direct_List.at(pos).readData(f);
                }
                else if (!m.direct_List.at(pos).exists(f)){
                ...
                myFile f(file, "");
                if(m.direct_List.at(pos).exists(f)) {
                    m.direct_List.at(pos).changeData(f, data);
             ...
             else if(buf1[0] == 'm' && buf1[1] == 'k' && buf1[2] == 'd' && buf1[3] == 'i' && buf1[4] == 'r') {
                string text = "";
                int j = 0;
                ...
                string msg = "";
                if(!m.directoryExists(d)) {
                    m.createDirectory(d);
                    msg += "\n0 Directory successfully created";
                }
                else if(m.directoryExists(d)) {
             ...
             else if(buf1[0] == 'c' && buf1[1] == 'd') {
                string text = "";
                int j = 0;
                for(int i = 3; i < strlen(buf1); i++) {
                    text += buf1[i];
                }
                text.erase(std::remove_if(text.begin(), text.end(), ::isspace), text.end());
                Directory d(text);
                ...
                if(m.directoryExists(d)) {
                    pos = m.getNewPos(d);
                    msg += "\n0 CURRENT DIRECTORY HAS BEEN CHANGED";
                    cout << m.direct_List.at(pos).getDirectoryName() << endl;
                }
                else {
                    msg += "\n1 ERROR: NO DIRECTORY EXISTS";
                }
                char message[msg.size() + 1];
                msg.copy(message, msg.size() + 1);
                message[msg.size()] = '\0';
             ...
             else if(buf1[0] == 'p' && buf1[1] == 'w' && buf1[2] == 'd') {
                string msg = "";
                msg += "\n";
                msg += m.direct_List.at(pos).getDirectoryName();
                char message[msg.size() + 1];
                msg.copy(message, msg.size() + 1);
                message[msg.size()] = '\0';
                send(newsock, message, strlen(message), 0);
             }
             else if(buf1[0] == 'r' && buf1[1] == 'm' && buf1[2] == 'd' && buf1[3] == 'i' && buf1[4] == 'r') {
                string text = "";
                int j = 0;
                for(int i = 6; i < strlen(buf1); i++) {
                    text += buf1[i];
